...
export default function AppView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const collectionData = await fetchCollectionData("PNR"); // Ganti "Pendapatan_Negara" dengan nama koleksi Anda di Firestore
        
        const mappedData = collectionData.map((doc, index) => ({
          id: index + 1, // Buat ID berdasarkan indeks (atau gunakan doc.id jika ada)
          Pendapatan_Negara: doc.Pendapatan_Negara || 0, // Pastikan ada fallback jika nilai tidak tersedia
  ...
  const Pendapatan_Negara = users.map(user => user.Pendapatan_Negara);
    ...
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={6} lg={8}>
          <AppWebsiteVisits
            title="Realisasi Pendapatan Negara Setiap Tahun"
            subheader="Asli"
            chart={{
