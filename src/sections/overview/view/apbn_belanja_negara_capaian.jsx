...
export default function AppView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const collectionData = await fetchCollectionData("BNC"); // Ganti "capaian" dengan nama koleksi Anda di Firestore
        
        const mappedData = collectionData.map((doc, index) => ({
          id: index + 1, // Buat ID berdasarkan indeks (atau gunakan doc.id jika ada)
          capaian: doc.capaian || 0, // Pastikan ada fallback jika nilai tidak tersedia
  ...
  const capaian = users.map(user => user.capaian);
    ...
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={6} lg={8}>
          <AppWebsiteVisits
            title="Target Capaian (%) Belanja Negara Setiap Tahun"
            subheader="Asli"
            chart={{
