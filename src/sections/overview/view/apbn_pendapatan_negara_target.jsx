...
export default function AppView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const collectionData = await fetchCollectionData("PNT"); // Ganti "pendapatan" dengan nama koleksi Anda di Firestore
        
        const mappedData = collectionData.map((doc, index) => ({
          id: index + 1, // Buat ID berdasarkan indeks (atau gunakan doc.id jika ada)
          pendapatan: doc.pendapatan || 0, // Pastikan ada fallback jika nilai tidak tersedia
  ...
  const pendapatan = users.map(user => user.pendapatan);
    ...
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={6} lg={8}>
          <AppWebsiteVisits
            title="Target Pendapatan Negara Setiap Tahun"
            subheader="Asli"
            chart={{
